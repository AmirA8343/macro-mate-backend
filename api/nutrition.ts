...
/** Final shape expected by your frontend */
...
/** Robust JSON extractor from LLM text */
...
/** Basic tokenizer overlap score for fuzzy matching */
...
/** Parse portion multiplier from text (e.g., '2 hash browns', 'x2', '2x') */
function parsePortionMultiplier(portionText?: string): number {
  ...
  // pick up 'large'/'medium'/'small' quick heuristics
...
/** Canonicalize item name for matching */
...
/* =================== SOURCE RESOLUTION =================== */
/** Score + pick the best Nutritionix branded candidate, then fetch details */
async function resolveBrandedFromNutritionix(itemText: string) {
  ...
  // score by overlap to prefer exact matches (e.g., “A&W Bacon & Egger”)
...
async function resolveGeneric(itemText: string) {
  ...
  let offItem = off?.products?.[0]?.nutriments;
  let offObj = offItem
    ? {
  ...
  // Prefer Nutritionix Natural if both exist (usually closer for cooked foods)
...
Your job is to FILL IN MICRONUTRIENTS ONLY and keep macros realistic.
Do NOT inflate macros. If any macros look extreme, normalize to these single-meal ranges:
...
`;
  const resp = await fetch("https://api.openai.com/v1/chat/completions", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${OPENAI_API_KEY}` },
    body: JSON.stringify({ model: "gpt-4o", messages: [{ role: "system", content: prompt }], temperature: 0 }),
  });
  const data = (await resp.json()) as OpenAIResponse;
  const content = data?.choices?.[0]?.message?.content ?? "";
  const parsed = extractJson(content) || {};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!NUTRITIONIX_APP_ID || !NUTRITIONIX_APP_KEY) {
    // still allow AI-only path if Nutritionix vars are missing
    console.warn("⚠️ Nutritionix keys missing; branded accuracy reduced.");
  }
  ...
  try {
    ...
    // if nothing detected, fallback to treating description as a single item
    const items = foods.length ? foods : [{ name: description || "meal", portion_text: "" }];

    /* -------- Stage 1.5: resolve each item via DBs (manager) -------- */
    // Parallel resolve per item (each item tries branded->generic)
    const resolved = await Promise.all(
      items.map(async (f) => {
        const baseText = canonicalizeName(f.name);
        const portionMult = parsePortionMultiplier(f.portion_text);
        // Try branded first
        const branded = NUTRITIONIX_APP_ID && NUTRITIONIX_APP_KEY ? await resolveBrandedFromNutritionix(baseText) : null;
        if (branded) {
          return {
            ...branded,
            // scale if portion says x2, etc.
        ...
        if (generic) {
          return {
            ...generic,
            // natural already considers portion words; we keep as is
            _isBranded: false,
          };
        }
        return null;
    ...
    // If we got at least one branded hit and verified totals look sane, skip GPT entirely.
    ...
    if (brandedCoverage >= 0.5 && looksSane) {
      // Return macros as-is, fill micros minimally with zeros (consistent schema)
    ...
    // If verified totals are 0 (DBs failed), still proceed (AI will estimate but clamped)
    ...
    merged.sodium   = clamp(merged.sodium,   0, 2000); // allow a bit higher here
    ...
    for (const k of nonNegKeys) {
      // @ts-ignore
      merged[k] = Math.max(0, Number(merged[k] || 0));
    }
